...
const getComments = (req, res) => {
    const q = "SELECT c.id, c.uidc, u.username, c.username, c.comment, u.img, c.date FROM users u JOIN comments c ON u.id = c.uidc JOIN posts p ON p.id = c.postId WHERE c.postId = ? AND c.parentId IS NULL";
...
const getReplyComment = (req, res) =>{
    const q = "SELECT c.id, c.uidc, u.username, c.username, c.comment, u.img, c.date, c.parentId FROM users u JOIN comments c ON u.id = c.uidc JOIN posts p ON p.id = c.postId WHERE c.postId = ? AND c.parentId IS NOT NULL";
    database.query(q, [req.query.postId], (err, data) => {
        if (err) return res.status(500).json(err);
        return res.status(200).json(data);
    });
...
const addReplyComments = (req, res) => {
    const token = req.cookies.access_token;
    if (!token) return res.status(401).json('Không được xác thực!');
    ...
    jwt.verify(token, 'jwtkey', (err, userInfo) => {
        ...
        const q = 'INSERT INTO comments(`comment`, `date`, `uidc`, `username`, `img`, `postId`, `parentId`, `answered`) VALUES (?)';
        const values = [
            req.body.comment,
            req.body.date,
            userInfo.id,
...
export { getComments, addComments, getReplyComment, addReplyComments, deleteComments, editComments }
