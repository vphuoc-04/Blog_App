...
export const getLikes = (req, res) => {
    const q = "SELECT userId, commentId FROM likes WHERE postId = ?";
...
export const addLike = (req, res) => {
    ...
    jwt.verify(token, 'jwtkey', (err, userInfo) => {
        ...
        const q = "INSERT INTO likes (`userId`, `postId`, `commentId`) VALUES (?, ?, ?)";
        const values = [
            userInfo.id,
            req.body.postId,
            req.body.commentId
...
export const deleteLike = (req, res) => {
    ...
    jwt.verify(token, 'jwtkey', (err, userInfo) => {
        if (err) { return res.status(403).json("Token không hợp lệ!"); }
        const q = "DELETE FROM likes WHERE `userId` = ? AND `postId` = ? AND `commentId` = ?";
